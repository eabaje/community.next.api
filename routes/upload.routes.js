...
const storageFromDisk = multer.diskStorage({
  destination: (req, res, cb) => {
    const { RefId, UploadType } = req.body;

    const dir = path.resolve(
      `${process.env.UPLOADS_URL}/${UploadType}/${RefId}`
    );
    fs.exists(dir, (exist) => {
      if (!exist) {
...
const storageFromMemory = multer.memoryStorage();
...
module.exports = function (app) {
  ...
  app.post(
    "/api/upload/uploadDocument",
    upLoadDocuments.single("doc"),
    controller.uploadDocument
  );

  app.post(
    "/api/upload/updateDocument",
    upLoadDocuments.single("doc"),
    controller.updateDocument
  );

  app.post(
    "/api/upload/uploadImageWithData",
    imageUploader.single("file"),
    controller.uploadImageWithData
  );

  app.post(
    "/api/upload/updateImageWithData",
    imageUploader.single("file"),
    controller.updateImageWithData
  );
